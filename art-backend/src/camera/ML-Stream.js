...
async function deleteAllFrameFolders() {
    try {
        ...
        // Check if the "frames" directory exists before trying to read it
        try {
            await fs.access(framesPath);
            ...
            if (err.code === 'ENOENT') {
                console.log(chalk.yellow('Frames directory does not exist yet, skipping cleanup'));
            } else {
